...
int main() 
{
    ...
    while(count<5000)
    {
        ...
        write(new_socket,&time_sent,sizeof(time_sent));
        
        valread = read(new_socket, &time_rec,sizeof(time_rec));
        
        clock_gettime(CLOCK_MONOTONIC,&t_received);
        delay = ((t_received.tv_sec)*pow(10,9)+t_received.tv_nsec)-time_rec;
        // printf("delay: %ld nanoseconds\n", delay);
